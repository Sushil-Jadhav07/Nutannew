...
const RenderedHighLightedBar: React.FC = () => {
    ...
    const HighLightedBarItems  = [
        {id: 1, title: "Updates to Umitex Digital Service Terms of Use and Privacy Policy ", href:"/"},
        {id: 2, title: "Welcome to Umitex Join us for 5% off & free delivery ", href:"/"},
          ...
          {highlightedBar !== 'true' && (
            <HighlightedBar  onClose={() => setHighlightedBar('true')}>
                ...
                >
                    {HighLightedBarItems.map((item, idx: number) => (
                        <SwiperSlide key={`highlightedBar--key-${idx}`}>
                            <div className="text-center text-base text-brand-white text-sm">
                                {item.title}
                                <Link className="notification-banner__link underline ml-2 hover:text-blue-500" to="{item.href}" >Learn More</Link>
                            </div>
                        </SwiperSlide>
                    ))
                    }
                ...
                <div className="hidden lg:block c-carousel-controls end-0 absolute top-1/2 -translate-y-1/2 me-12  " >
                    <div className="bg-black/20 flex rtl:flex-row-reverse items-center justify-center gap-1 text-brand-white p-1 rounded-full">
                        <button
