...
const ListingFeatured: FC<BoxProps> = ({
    ...
    const { data: products, isLoading, error } = useFirebaseBestSellerProducts(LIMITS.BEST_SELLER_PRODUCTS_LIMITS);
    ...
    const filteredProducts = () => {
        switch (activeTab) {
            case 'featured':
                return products?.filter((product: Product) => product.isFeatured); // Assuming `isFeatured` is a boolean
            case 'topTrending':
                return products?.sort((a: Product , b: Product) => b.rating - a.rating); // Assuming rating filter
            case 'onSale':
                return products?.filter((product: Product) => product.discountPercentage > 0); // Assuming discount filter
            default:
                return products;
        }
    };
     ...
     // Log for debugging
     console.log('🔄 ListingFeatured render:', { 
         activeTab,
         productCount: products?.length || 0, 
         filteredCount: displayProducts?.length || 0,
         isLoading, 
         hasError: !!error,
         error: error?.message 
     });
