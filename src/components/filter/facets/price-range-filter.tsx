...
export function PriceRangeFilter({ min, max, value, onChange }: PriceRangeFilterProps) {
    // Ensure we always start with 0 to max range
    const initialValue: [number, number] = [0, max];
    const [localValue, setLocalValue] = useState<[number, number]>(initialValue)
    const [inputMin, setInputMin] = useState<string>(initialValue[0].toString())
    const [inputMax, setInputMax] = useState<string>(initialValue[1].toString())
    
    // Update local state when props change
    useEffect(() => {
        // If value is the default range, use 0 to max
    ...
    const handleMinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isNaN(numValue)) {
            const validMin = Math.max(0, Math.min(numValue, localValue[1] - 1))
