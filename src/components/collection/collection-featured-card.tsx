        ...
        >
            ...
            <div className="absolute  p-4 lg:p-9 left-5 bottom-5 lg:left-10 lg:bottom-10 bg-white rounded-md max-w-xs sm:max-w-sm">
                ...
                {btnText && (
                    <Link
                        to={btnUrl as string}
                        className={cn(colorMap[selectedColor].hoverBg,colorMap[selectedColor].hoverBorder,'border-2 border-brand-dark h-[44px] mt-5 md:mt-8 text-sm font-bold text-brand-dark inline-flex transition duration-300 rounded px-10 py-2  sm:hover:text-brand-light')}
                    >
                        {btnText}
                    </Link>
                )}
