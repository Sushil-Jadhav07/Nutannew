...
const AccountNavMobile: React.FC<{ options: Option[] }> = ({ options }) => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Equivalent of usePathname: using location.pathname from react-router-dom
  const pathname = location.pathname;
  const pathnameSplit = pathname.split('/');
  const newPathname: string = `/${pathnameSplit.slice(1).join('/')}`;
